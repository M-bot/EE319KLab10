...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	PUSH {R4, R5}
    LDR R5,=SSI0_SR_R
BW	LDR R4,[R5]				; wait for device
	ANDS R4, #SSI_SR_BSY
	BNE	BW
	
	LDR R5,=DC				; set to command
	LDR R4,[R5]
	BIC R4, #DC_DATA
	STR R4,[R5]
	
	LDR R5,=SSI0_DR_R		; send command
	STR R0,[R5]
	
	LDR R5,=SSI0_SR_R		; wait for device
BW2	LDR R4,[R5]
	ANDS R4, #SSI_SR_BSY
	BNE	BW2
...
;4) Write the 8-bit data to SSI0_DR_R
	PUSH {R4, R5}
	LDR R5,=SSI0_SR_R
BW3	LDR R4,[R5]				; wait for device
	ANDS R4, #SSI_SR_TNF
	BEQ	BW3
    
    LDR R5,=DC				; set to data
	LDR R4,[R5]
	ORR R4, #DC_DATA
	STR R4,[R5]
	
	LDR R5,=SSI0_DR_R		; send data
	STR R0,[R5]
